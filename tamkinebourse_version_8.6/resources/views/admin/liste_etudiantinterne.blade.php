...
<h3 class="text-center"> Recherche élèves Marocains </h3>

<div class="container">
<form action="/inscriptioninterne" method="POST">
        {{ csrf_field() }}
    ...
    <form action="{{ route('export_excel.in') }}" method="POST">
    {{ csrf_field() }}
  ...
  <table class="table table-striped uk-table">
        <thead>
          <tr>
            ...
            <th class="table-active">Téléphone</th>
        ...
        <tbody>
                <tr>
                    @foreach ($ins as $in)
                <td class="table-primary">{{$in->cin}}</td>
                <td class="table-primary">{{$in->user->prenom}}</td>
                <td class="table-primary">{{$in->user->nom}}</td>
                <td class="table-primary">{{$in->date_naissance}}</td>
                <td class="table-primary">{{$in->telephone}}</td>
                <td class="table-primary">{{$in->user->ville}}</td>
                <td class="table-primary">{{$in->user->adresse}}</td>
                <td class="table-primary">{{$in->user->email}}</td>
                <td class="table-primary">{{$in->etablissement}}</td>
                <td class="table-primary">{{$in->filiere_bac}}</td>
                <td class="table-primary">{{$in->aref->nom_aref}}</td>
                <td class="table-primary">{{$in->direction->nom_direction}}</td>
                <td class="table-primary">{{$in->note_premierbac}}</td>
                <td class="table-primary">{{$in->note_examreg}}</td>
                <td class="table-primary">{{$in->type_bourse}}</td>
                <td class="table-primary">{{$in->universite->nom_universite}}</td>
                <td class="table-primary">{{$in->domaine->nom_domaine}}</td>
                <td class="table-primary">{{$in->filiere->nom_filiere}}</td>
                <td class="table-primary">{{$in->annee_inscription}}</td>
                <td class="table-primary">{{$in->situation_pere}}</td>
                <td class="table-primary">{{$in->situation_mere}}</td>
                <td class="table-primary">
                    <a href='{{$in->featured_1}} ' download="{{$in->featured_1}}">
                        <button class="btn btn-primary"><i class="fa fa-download" aria-hidden="true"></i> Situation.Père </button>
                      </a>
                </td>
                <td class="table-primary">
                    <a href='{{$in->featured_2}}' download="{{$in->featured_2}}">
                        <button class="btn btn-primary"><i class="fa fa-download" aria-hidden="true"></i> Situation.Mère </button>
                      </a>
                </td>
                <td class="table-primary">
                    <a href='{{$in->featured_3}}' download="{{$in->featured_3}}">
                        <button class="btn btn-primary"><i class="fa fa-download" aria-hidden="true"></i> Premier.Bac </button>
                    </a>
                </td>
                <td class="table-primary">
                <a href= "{{$in->featured_4}}" download="{{$in->featured_4}}">

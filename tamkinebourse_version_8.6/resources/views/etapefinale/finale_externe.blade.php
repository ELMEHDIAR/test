...
<h3 class="text-center"> Liste Etudiants Étrangère </h3>

<div class="container">
<form action="/externe_search_final" method="POST">
        {{ csrf_field() }}
    ...
    {{--  <table>
     ...
     <!-- <td><input type="text"  name="note_bac1" id="note_bac1" class="form-control" placeholder="note bac min"></td>
     <td><input type="text"  name="note_bac2" id="note_bac2" class="form-control" placeholder="note bac max"></td> -->
    ...
    <form action="{{ route('export_excel.finale') }}" method="POST">
    {{ csrf_field() }}
  ...
  <table class="table table-striped uk-table">
        ...
        <tbody>
            @foreach ($ext as $s)
                ...
                <td class="table-primary">{{$s->pays}}</td>
